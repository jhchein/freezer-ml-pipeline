...
def get_connection_device_id(data):
    ...
    unique_connection_device_ids = list(set([event.get("ConnectionDeviceId") 
                                            for event in data.get("allevents") 
                                            if event.get("ConnectionDeviceId") is not None]))

    if len(unique_connection_device_ids) > 1:
        error_message = f"Multiple ConnectionDeviceIds found ({unique_connection_device_ids})."
        logging.warning(error_message)
        return None, True, error_message
    if len(unique_connection_device_ids) < 1:
        error_message = f"No ConnectionDeviceIds found."
        logging.warning(error_message)
        return None, True, error_message
